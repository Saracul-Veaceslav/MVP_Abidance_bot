...
- Follow a Test-Driven Development (TDD) approach: write tests first, then implement features
- All code must have corresponding tests before being considered complete
- Each test must include a Gherkin-style docstring explaining the test purpose
- A task is considered complete only when:
  1. All tests are passing (green)
  2. The code has been formatted with Black
  3. The code has been linted with Pylint and achieves a score higher than 9.0
- Test files should follow the same naming convention as the files they test (e.g., `example-component.test.tsx` for `example-component.tsx`)
- For UI components, use React Testing Library
- For server actions, use Jest
- For integration tests, use Cypress
